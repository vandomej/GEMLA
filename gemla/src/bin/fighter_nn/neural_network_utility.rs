...
pub fn crossbreed(
    fighter_nn: &FighterNN,
    primary: &Fann,
    secondary: &Fann,
    crossbreed_segments: usize,
) -> Result<Fann, Error> {
    ...
    let mut new_fann =
        Fann::new(new_shape.as_slice()).with_context(|| "Failed to create new fann")?;
    // We need to randomize the weights to a small value
    new_fann.randomize_weights(
        fighter_nn.weight_initialization_range.start,
        fighter_nn.weight_initialization_range.end,
    );
...
pub fn generate_segments(
    primary_shape: Vec<u32>,
    secondary_shape: Vec<u32>,
    crossbreed_segments: usize,
) -> Vec<(u32, u32)> {
    ...
    let end = min(
        primary_shape.iter().sum::<u32>() - primary_shape.last().unwrap(),
        secondary_shape.iter().sum::<u32>() - secondary_shape.last().unwrap(),
    );
...
pub fn consolidate_old_connections(
    primary: &Fann,
    secondary: &Fann,
    new_shape: Vec<u32>,
    new_neurons: Vec<(u32, bool, usize, u32)>,
    new_fann: &mut Fann,
) {
    ...
    for layer in 1..new_shape.len() {
        ...
        let current_layer_connections = new_neurons
            .iter()
            .filter(|(_, _, l, _)| l == &layer)
            .collect::<Vec<_>>();
        let previous_layer_connections = new_neurons
            .iter()
            .filter(|(_, _, l, _)| l == &(layer - 1))
            .collect::<Vec<_>>();
        ...
        for (neuron_id, is_primary, _, new_id) in current_layer_connections.iter() {
            ...
            for (previous_neuron_id, _, _, previous_new_id) in previous_layer_connections.iter() {
                ...
                if *is_primary {
                    connection = primary_connections.iter().find(|connection| {
                        let from_neuron =
                    ...
                    if connection.is_none() {
                        connection = secondary_connections.iter().find(|connection| {
                            let from_neuron =
        ...
        if let Some(bias_neuron) = bias_neuron {
            ...
            for (neuron_id, is_primary, _, new_id) in current_layer_connections.iter() {
                ...
                if *is_primary {
                    ...
                    if let Some(primary_bias_neuron) = primary_bias_neuron {
                        connection = primary_connections.iter().find(|connection| {
                            ...
                            if let Some(to_neuron) = to_neuron {
                                connection.from_neuron == primary_bias_neuron
                                    && to_neuron == *neuron_id
                            } else {
                                false
                            }
                    ...
                    if connection.is_none() {
                        let secondary_bias_neuron =
                            get_bias_neuron_for_layer(layer, &secondary_shape);
                        if let Some(secondary_bias_neuron) = secondary_bias_neuron {
                            connection = secondary_connections.iter().find(|connection| {
                                ...
                                if let Some(to_neuron) = to_neuron {
                                    connection.from_neuron == secondary_bias_neuron
                                        && to_neuron == *neuron_id
                                } else {
                                    false
                                }
...
pub fn crossbreed_neuron_arrays(
    segments: Vec<(u32, u32)>,
    primary_neurons: Vec<(u32, usize)>,
    secondary_neurons: Vec<(u32, usize)>,
) -> Vec<(u32, bool, usize, u32)> {
    ...
    for (i, &segment) in segments.iter().enumerate() {
        ...
        if i == 0 {
            ...
            let target_neurons = if is_primary {
                &primary_neurons
            } else {
                &secondary_neurons
            };
            ...
            for (neuron_id, layer) in target_neurons.iter() {
                ...
                if neuron_id >= &segment.0 && neuron_id <= &segment.1 {
                    ...
                    // Earlier
                        ...
                        let earlier_layer_neurons = new_neurons
                            .iter()
                            .filter(|(_, _, l, _)| l == layer)
                            .collect::<Vec<_>>();
                    ...
                    // Later
                        ...
                        let current_layer_neurons = new_neurons
                            .iter()
                            .filter(|(_, _, l, _)| l == &current_layer)
                            .collect::<Vec<_>>();
                        let highest_id =
                        ...
                        let neurons_to_add = target_neurons
                            .iter()
                            .filter(|(id, l)| id <= neuron_id && l == layer)
                            .collect::<Vec<_>>();
                        for (neuron_id, _) in neurons_to_add {
                            new_neurons.push((*neuron_id, is_primary, current_layer, 0));
        ...
        if neuron_id > &segments.last().unwrap().1 {
            if layer == &output_layer {
                ...
                let target_layer = if is_primary {
                    ...
                    secondary_last_layer
                };
                let earlier_layer_neurons = new_neurons
                    .iter()
                    .filter(|(_, _, l, _)| *l >= target_layer && l <= layer)
                    .collect::<Vec<_>>();
    ...
    let layer_counts = new_neurons.iter().fold(
        vec![0; current_layer + 1],
        |mut counts, &(_, _, layer, _)| {
            counts[layer] += 1;
            counts
        },
    );
    ...
    let layer_mapping = unique_layers
        .iter()
        .enumerate()
...
pub fn major_mutation(fann: &Fann, weight_initialization_range: Range<f32>) -> Result<Fann, Error> {
    ...
    let mut mutated_neurons = generate_neuron_datastructure(&mutated_shape)
        .iter()
        .map(|(id, layer)| (*id, true, *layer, *id))
        .collect::<Vec<_>>();
    ...
    let mut mutated_fann =
        Fann::new(mutated_shape.as_slice()).with_context(|| "Failed to create new fann")?;
    mutated_fann.randomize_weights(
    ...
    for (i, (_, _, _, new_id)) in mutated_neurons.iter_mut().enumerate() {
        *new_id = i as u32;
    }
...
fn to_bias_network_id(id: &u32, shape: &[u32]) -> u32 {
...
fn to_non_bias_network_id(id: u32, shape: &[u32]) -> Option<u32> {
    ...
    for (bias_count, &neurons) in shape.iter().enumerate() {
        ...
        if id < layer_end {
            ...
            return Some(id - bias_count as u32);
        }
        if id == layer_end {
...
fn get_bias_neuron_for_layer(layer: usize, shape: &[u32]) -> Option<u32> {
    if layer == 0 || layer >= shape.len() {
        ...
        let mut bias = 0;
        for layer_count in shape.iter().take(layer) {
            bias += layer_count;
        }
...
mod tests {
    ...
    fn major_mutation_test() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for _ in 0..100 {
            ...
            assert_eq!(
                primary_shape[primary_shape.len() - 1],
            ...
            if new_shape.iter().sum::<u32>() == primary_shape.iter().sum::<u32>() + 1 {
                ...
                for connection in connections.iter() {
                    if connection.from_neuron == added_neuron_id
                        || connection.to_neuron == added_neuron_id
                    {
                        assert!(
                            connection.weight < 0.0,
                            "Connection: {:?}, Added Neuron: {}",
                            connection,
                            added_neuron_id
                        );
                    } else {
                ...
                for (i, layer) in new_shape.iter().enumerate() {
                    ...
                    if i != 0 && i as u32 != new_shape.len() as u32 - 1 {
                        assert!(
                            *layer >= NEURAL_NETWORK_HIDDEN_LAYER_SIZE_MIN as u32,
                            "Layer: {}",
                            layer
                        );
                    }
    ...
    fn generate_segments_test() {
        ...
        // Assert
        assert!(
            result.len() <= crossbreed_segments,
            "Segments: {:?}",
            result
        );
    ...
    fn crossbreed_neuron_arrays_test() {
        ...
        // Act
        let result = crossbreed_neuron_arrays(
            segments.clone(),
            primary_network.clone(),
            secondary_network.clone(),
        );
    ...
    fn consolidate_old_connections_test() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for connection in expected_connections.iter() {
            let matching_connection = new_connections
                .iter()
                .find(|&c| c.from_neuron == connection.0 && c.to_neuron == connection.1);
            if let Some(matching_connection) = matching_connection {
                assert_eq!(
                    matching_connection.weight, connection.2,
                    "Connection: {:?}",
                    matching_connection
                );
            } else {
